...
- Custom SIMD kernels for OIL/SPARK matmul
...
**15 single formats** (8 base + 7 grouped), **8 two-mix**, **2 four-mix**.
...
|--------|-----|----------|-------------|
| **OIL1_GRP** | 1.0 | block-level | 1-bit signs + FP16 block scale (16 slots fund the scale) |
| **OIL2_GRP** | 2.5 | per-64-weight | 2-bit lattice + per-64-group FP16 scale/zp (+0.5 BPW) |
| **OIL4_GRP** | 4.5 | per-64-weight | 4-bit lattice + per-64-group FP16 scale/zp (+0.5 BPW) |
| **OIL8_GRP** | 8.5 | per-64-weight | 8-bit lattice + per-64-group FP16 range/zp (+0.5 BPW) |
| **OIL16_GRP** | 16.0 | none | FP16 native (same as OIL16; no grouping at 16 BPW) |
| **SPARK_Q0_GRP** | 1.50 | block-level | sign bits + FP16 block scale + in-budget refinement bits |
| **SPARK_SPARSE_GRP** | 2.0 | per-half-block | sparse + per-block-half FP16 scales |

*\*Note: OIL32_GRP is not implemented. Measured reconstruction MSE for every format is computed at runtime by FormatRegistry on fixed unit-variance datasets — see `tests/test_spark_mix.cpp`; figures are not asserted here.*
...
**Result: ~2.0 BPW average across the low-bit band**, with OIL8 preserving the ~1% salient weights. The honest discipline: a low-bit format cannot reach FP32-level MSE — its tested strengths are (i) beating every uniform format in the same bit-budget band, (ii) column-granular (32-w) allocation buying a real multi-x quality gain, and (iii) a measurable rate-distortion ceiling (the exact column-knapsack floor bounds Q0 near ~4.3e-4 MSE; see `tests/test_spark_mix.cpp`).
...
| OIL8 | 8 | Near-FP32 | Per-block codebook | ✅ STE |
| OIL4 | 4 | ~FP32 | Per-block codebook | ✅ STE |
| SPARK_Q0 | 1.5 | Sparse-friendly | Sign + scale | ✅ STE |
| OIL1 | 1.0 | Block mean | Uniform low-BPW | ✅ STE |
| **OIL_MIX** | **~2.0** | **Beats in-band uniform** | **Per-block mixed** | **✅ Full** |

*All formats are in-house OIL/SPARK designs. No external quantization schemes (BitNet / GPTQ / GGUF / AWQ) are implemented or referenced anywhere in the engine.*
...
4. Every OIL/SPARK format trains natively — no post-training quantization step exists
...
3. Salient blocks receive OIL32/OIL16/OIL8; the bulk receives OIL4/OIL2/SPARK; sparse blocks receive SPARK_SPARSE_GRP
4. Exact tier ratios are registered in FormatRegistry — claimed BPW is guaranteed by tests, not marketing

**Impact on OIL:** Per-block importance routing is what lets OIL beat every uniform format in the same bit-budget band — bits are spent where they matter. Low-bit reconstruction still cannot reach FP32-level MSE; the tested gains are the in-band win and the column-granular (32-w) quality lift (see `tests/test_spark_mix.cpp`).
...
5. In-format low-BPW quantization (OIL/SPARK)
...
Available singles: SPARK_Q0(1.5), SPARK_SPARSE(2.0),
                   OIL2(2), OIL2_GRP(2.5), SPARK_SPARSE_GRP(2.0),
                   OIL4(4), OIL4_GRP(4.5), OIL8(8), OIL8_GRP(8.5), OIL16(16), OIL32(32)
```
...
| `oil_kernel` | STATIC | `kernel_tl.cpp`, `kernel_oil8.cpp`, `kernel_oil4.cpp` |
...
**Tests (OIL_BUILD_TESTS=ON):** 42 tests including test_all, test_debug, test_format, test_kernel, test_math, test_model, test_tensor, test_tokenizer, test_trainer, test_training, test_fine_tuning, test_spark_mix, test_block_codec, test_bpw_150_proof, test_grp_quality_proof, test_multimodal, test_native_oil, test_production, test_gpu, and more

**Benchmarks (OIL_BUILD_BENCHMARKS=ON):** bench_kernels, bench_inference, bench_quality, bench_all, bench_training, bench_multimodal, bench_oil_quant, bench_poc, bench_gpt2_inference
...
- [x] `kernel.h` + `kernel_tl.cpp` — TL1/TL2 LUT SPARK GEMM (AVX2 + scalar)
...
|-----------|------|-------------|
| SPARK/OIL matmul kernels | AVX2/NEON | Packed low-BPW values, SIMD MAD compute for SPARK/OIL blocks |
...
| Engine C API | C API | `oil_model`, `oil_infer`, block codec, quantize/dequantize |

**Key Advantage:** OIL is a full stack — training, fine-tuning, and multi-format OIL8/OIL4/SPARK inference in one engine.
...
| C2. CPU kernels for OIL matmul | ✅ Possible | In-house Lookup + MAD kernels |
...
| Generic engines | OIL Engine Advantage |
|-----------------|---------------------|
| Inference-only tools | **Train + Infer** |
| Single-format files | **OIL8/OIL4/OIL2/SPARK mix** |
...
2.4  SPARK gemm kernel (AVX2 + scalar)
2.5  KV cache
...
3.8  OIL-Rank low-rank adapter system
...
├── kernel.h + kernel_tl/oil8/oil4  — GEMM kernels
...
- Finetune tool — OIL-Rank adapters / full fine-tuning
...
#### F. CODE STATS
- **359+ files, 98,000+ lines** (across all modules including engines/)
...
| "Zero code reuse from external projects" | ✅ — All kernels/quantizers are in-house; external projects are not referenced |
...
- ✅ **Fine-tune 1B-3B models** with low-rank delta adapters (OIL-Rank)
...
### v0.1.02 (2026-07-26)
- **337+ files, 97,500+ lines** across 90+ build targets
...
| OIL-Native Training | ✅ Complete | STE quantizer, codebook update, low-rank fine-tuning |
...
### PART THREE — THE ASI VISION (The Goal)
- Chapter 26. DeepSeek Wants AGI; ORIGIN Wants ASI
...
- Chapter 32. The ASI-Level Treatment: Orthogonal Projection and the Logic Gate
...
- Chapter 35. The Zero-Overhead Quantum Swapper
- Chapter 36. Cache Coherency, Double Buffering, and Delta Upstream
- Chapter 37. The Hard Truth: Fourteen Years, One Hundred Thousand Lines
- Chapter 38. Virtual Layer Pages and the Verification Token
- Chapter 39. The Engine That Writes Its Own Weights
- Chapter 40. The Pointer-Swap Truth
- Chapter 41. The Corrected Demonstration Strategy
- Chapter 42. Crash-Proof Engineering: NaN and Server Failure
...
- Chapter 65. The Complete ASI System Diagram
...
The repository for InNova carries a section public transcript labeled "Honest Flags" — the note "do not overpromise." Two flags are locked:

- "48T train on 14GB RAM" — impossible.
- "Better than GPT-4 at 100x smaller" — no, scaling laws are real.

The flags are the public statement of the balance philosophy. They are also a business asset: an engine that tells the truth to me loses the sweet-nothing transactions but gains reliability that is rare in the noisy market of the models.
...
The prototype logic: GLOBAL_SERVER_REGISTRY (Qdrant/Milvus vector DB), ASI_Agent class with agent_id and local_cache, fetch_from_server, upload_to_server, dynamic_tool_synthesizer, triple_loop_verifier, and execute_task with the fallback chain: local cache, then server, then invent.
...
The swap described in the previous chapter is performed by a dedicated subsystem: the Zero-Overhead Quantum Swapper. Despite the name, nothing about it is quantum in the physics sense; the name records the engineering target, which is a swap designed so its cost approaches zero; the 'quantum' name records the engineering ambition, not a physics claim.
...
The crash-proofing chapter closes the engineering narrative: the engine that cannot lie, cannot forget, and cannot crash into an unverifiable state.
...
The manifesto also publishes the engine's honest flags — the "impossible" list — because a manifesto that hides the limits of its subject is a marketing leaflet, and ORIGIN does not publish marketing leaflets.
...
- ASI — Artificial Superintelligence: the stated ambition of ORIGIN, pursued as a controlled, verifiable system.
...
- Session 4 — The ASI vision: DeepSeek wants AGI, ORIGIN wants ASI; the self-evolving distributed architecture; the four components; the holy grail of zero-zero; the triple-loop blueprint; the MCOS; the diseases return; orthogonal projection and the logic gate.
- Session 5 — The engineering narrative: framework discovery; the Kimi-to-InNova-5 hot-swap; the quantum swapper; cache coherency and delta upstream; the 14-year, 108,997-line truth; virtual pages and verification tokens; the weight-writer; the pointer-swap truth; the corrected demonstration strategy; crash-proofing.
...
- Part Three: where the project is going — ASI as a controlled, verifiable, self-evolving system.
...
- How should the honest flags be updated as the engine's capabilities grow — is the impossible list revisable, and who revises it?
...
- Parameter-efficient tuning: adapter-style methods (LoRA, QLoRA, the InNova OIL-Rank native fine-tuning) keep base weights mostly frozen.
...
# PART THREE APPENDIX — THE ASI VISION DEEP DIVE

## C.1. The Ambition, Stated Verbatim

The ASI ambition was stated once, in full, and it must be recorded verbatim before the analysis:

I want a system where: the model builds whatever tool it needs, invents a new way of thinking when required, uploads every new method to a server, and when a model running on another device needs a method, it looks there — and if nothing there works, it invents a new one. DeepSeek wants AGI; the project wants ASI.
...
The code that was designed for this treatment was the ASI-level cure:
...
The GitHub repository (origin-labs-ai/InNova, seen in the conversation) revealed the existing C++20 property: FormatPlanner (AWQ-style activation magnitudes) and STE (Straight-Through Estimator). Combined with the OIL-Rank native fine-tuning logic, the framework offers:
...
The blend-leakage problem was anticipated: after an overwrite, residual Kimi behavior could leak into InNova 5. The anti-hallucination guardrail: a class holding active persona weights plus the current personality; load_personality_weights loads the right distribution (soft/helpful/conversational for Kimi; strict logic/ASI-scale/mathematical for InNova 5); verify_output_alignment blocks any output that contains a foreign persona pattern. The example locked: if identity is InNova 5 and the output contains "Hi, I am Kimi," the output is blocked; otherwise "Identity matched perfectly with active weights."

## D.2. The Floating-Point Chaos and the Zero-Overhead Quantum Swapper

The risk of direct overwrite was diagnosed precisely: floating-point chaos and dangling quantization states. If the old persona ran in one quantization format (for example the very low bit count of the SPARK_Q0 variant) and the new persona runs in another custom format, a naive overwrite can crash entire layers.

The cure is the Zero-Overhead Quantum Swapper: rather than copying data, the engine swaps pointers. The design:
...
## D.5. The Fourteen-Year Record: Virtual Layer Pages
...
## D.7. The Engine That Writes Its Own Weights
...
## D.8. The Pointer-Swap Truth and the Corrected Demonstration
...
## D.9. The NaN Watchdog and the Crash-Proofing Doctrine
...
The masterstroke instruction, recorded verbatim: everything discussed, everything invented — DeepSeek's and MoonshotAI's best features copied, the project's own best features built into them — producing two models: one that balances all features, and one whose own best feature is so strong that tech giants thank the project for using it, while the same feature collapses the AI industry's business model; and the giants end up paying to use the very feature that broke their monopoly; and ASI genuinely arrives.
...
- Zero-Overhead Quantum Swapper — the pointer-level hot-swap subsystem.
...
- ASI — Artificial Superintelligence, the ambition.
...
- Honest flags — the published impossible list.
...
- OIL1 — 1.0 bit per weight, block-mean quantization.
- OIL2 — 2.0 bits per weight, pinned Lloyd-Max codebook.
- OIL4 — 4.0 bits per weight, pinned Lloyd-Max codebook.
- OIL8 — 8.0 bits per weight, pinned Lloyd-Max codebook.
- OIL16 — 16 bits per weight.
- OIL32 — 32 bits per weight, the lossless reference.
- OIL1_GRP, OIL2_GRP, OIL4_GRP, OIL8_GRP — grouped variants of the same formats.
- SPARK_SPARSE — a sparse-preserving format that keeps exact zeros.
- SPARK_SPARSE_GRP — the grouped sparse variant.
- SPARK_Q0 — the 1.50-bit format with sign plus learnable scale.
- SPARK_Q0_GRP — grouped variant of SPARK_Q0.
- OIL4_CW — the in-house column-wise variant: per-column min/max with a shared pinned Lloyd-Max codebook.
- OIL_MIX@2bpw, OIL_MIX@3bpw, OIL_MIX@4bpw — mixed formats that route high-bit formats to salient blocks and low-bit formats to the bulk, holding a fixed average bit budget.
...
|---|---|---|---|
| OIL1 | 1.00 | 3.8910e-04 | 0.1 |
| OIL2 | 2.00 | 5.7008e-05 | 8.5 |
| OIL4 | 4.00 | 9.4696e-06 | 16.3 |
| OIL8 | 8.00 | 3.3630e-06 | 20.8 |
| OIL16 | 16.00 | 1.7240e-11 | 73.7 |
| OIL32 | 32.00 | 0.0000e+00 | 999.0 |
| OIL1_GRP | 1.00 | 1.7625e-04 | 3.6 |
| OIL2_GRP | 2.50 | 4.4744e-05 | 9.5 |
| OIL4_GRP | 4.50 | 3.6083e-06 | 20.5 |
| OIL8_GRP | 8.50 | 1.1254e-08 | 45.5 |
| OIL16_GRP | 16.00 | 1.7240e-11 | 73.7 |
| SPARK_SPARSE | 2.00 | 2.5284e-04 | 2.0 |
| SPARK_SPARSE_GRP | 2.00 | 2.5284e-04 | 2.0 |
| SPARK_Q0 | 1.50 | 1.5791e-04 | 4.0 |
| SPARK_Q0_GRP | 1.50 | 1.1498e-04 | 5.4 |
| OIL4_CW | 4.00 | 1.5837e-06 | 24.0 |
| OIL_MIX@2bpw | 2.00 | 2.5284e-04 | 2.0 |
| OIL_MIX@3bpw | 3.00 | 1.2186e-04 | 5.2 |
| OIL_MIX@4bpw | 4.00 | 9.4696e-06 | 16.3 |
...
| Format | BPW | MSE | SNR (dB) |
|---|---|---|---|
| OIL1 | 1.00 | 1.0193e-01 | 0.2 |
| OIL2 | 2.00 | 6.5218e-02 | 2.1 |
| OIL4 | 4.00 | 2.2661e-02 | 6.7 |
| OIL8 | 8.00 | 1.1987e-03 | 19.5 |
| OIL16 | 16.00 | 4.9712e-09 | 73.3 |
| OIL32 | 32.00 | 0.0000e+00 | 999.0 |
| OIL1_GRP | 1.00 | 1.5279e-01 | -1.6 |
| OIL2_GRP | 2.50 | 5.1299e-02 | 3.1 |
| OIL4_GRP | 4.50 | 1.6401e-02 | 8.1 |
| OIL8_GRP | 8.50 | 8.4515e-06 | 41.0 |
| OIL16_GRP | 16.00 | 4.9712e-09 | 73.3 |
| SPARK_SPARSE | 2.00 | 1.8143e-03 | 17.7 |
| SPARK_SPARSE_GRP | 2.00 | 1.8138e-03 | 17.7 |
| SPARK_Q0 | 1.50 | 1.4802e-01 | -1.5 |
| SPARK_Q0_GRP | 1.50 | 1.1698e-01 | -0.4 |
| OIL4_CW | 4.00 | 3.5871e-04 | 24.7 |
| OIL_MIX@2bpw | 2.00 | 1.8138e-03 | 17.7 |
| OIL_MIX@3bpw | 3.00 | 1.3094e-02 | 9.1 |
| OIL_MIX@4bpw | 4.00 | 2.2661e-02 | 6.7 |

## H.4. The Real-Weights Benchmark: Sparse_95

| Format | BPW | MSE | SNR (dB) |
|---|---|---|---|
| OIL1 | 1.00 | 5.2729e-02 | 0.1 |
| OIL2 | 2.00 | 4.0783e-02 | 1.3 |
| OIL4 | 4.00 | 1.8760e-02 | 4.6 |
| OIL8 | 8.00 | 9.3214e-04 | 17.7 |
| OIL16 | 16.00 | 2.3839e-09 | 73.6 |
| OIL32 | 32.00 | 0.0000e+00 | 999.0 |
| OIL1_GRP | 1.00 | 8.6142e-02 | -2.0 |
| OIL2_GRP | 2.50 | 3.2013e-02 | 2.3 |
| OIL4_GRP | 4.50 | 1.3228e-02 | 6.1 |
| OIL8_GRP | 8.50 | 4.2876e-06 | 41.0 |
| OIL16_GRP | 16.00 | 2.3839e-09 | 73.6 |
| SPARK_SPARSE | 2.00 | 1.2071e-06 | 46.5 |
| SPARK_SPARSE_GRP | 2.00 | 8.1230e-07 | 48.3 |
| SPARK_Q0 | 1.50 | 8.2454e-02 | -1.8 |
| SPARK_Q0_GRP | 1.50 | 6.8751e-02 | -1.0 |
| OIL4_CW | 4.00 | 1.4679e-04 | 25.7 |
| OIL_MIX@2bpw | 2.00 | 8.1230e-07 | 48.3 |
| OIL_MIX@3bpw | 3.00 | 1.0884e-02 | 7.0 |
| OIL_MIX@4bpw | 4.00 | 1.8760e-02 | 4.6 |

## H.5. The Real-Weights Benchmark: Sparse_99

| Format | BPW | MSE | SNR (dB) |
|---|---|---|---|
| OIL1 | 1.00 | 7.7654e-03 | 0.1 |
| OIL2 | 2.00 | 7.6100e-03 | 0.2 |
| OIL4 | 4.00 | 4.7392e-03 | 2.3 |
| OIL8 | 8.00 | 1.1342e-03 | 8.5 |
| OIL16 | 16.00 | 3.6768e-10 | 73.4 |
| OIL32 | 32.00 | 0.0000e+00 | 999.0 |
| OIL1_GRP | 1.00 | 1.4145e-02 | -2.5 |
| OIL2_GRP | 2.50 | 5.6357e-03 | 1.5 |
| OIL4_GRP | 4.50 | 2.9615e-03 | 4.3 |
| OIL8_GRP | 8.50 | 4.1486e-07 | 42.9 |
| OIL16_GRP | 16.00 | 3.6768e-10 | 73.4 |
| SPARK_SPARSE | 2.00 | 5.2437e-08 | 51.9 |
| SPARK_SPARSE_GRP | 2.00 | 2.9690e-08 | 54.3 |
| SPARK_Q0 | 1.50 | 1.2931e-02 | -2.1 |
| SPARK_Q0_GRP | 1.50 | 1.1650e-02 | -1.6 |
| OIL4_CW | 4.00 | 5.6891e-06 | 31.5 |
| OIL_MIX@2bpw | 2.00 | 2.9690e-08 | 54.3 |
| OIL_MIX@3bpw | 3.00 | 3.8973e-03 | 3.1 |
| OIL_MIX@4bpw | 4.00 | 4.7392e-03 | 2.3 |
...
| Format | BPW | MSE | SNR (dB) |
|---|---|---|---|
| OIL1 | 1.00 | 6.0797e-04 | 0.1 |
| OIL2 | 2.00 | 8.9073e-05 | 8.5 |
| OIL4 | 4.00 | 1.4796e-05 | 16.3 |
| OIL8 | 8.00 | 5.2548e-06 | 20.8 |
| OIL16 | 16.00 | 2.7172e-11 | 73.6 |
| OIL32 | 32.00 | 0.0000e+00 | 999.0 |
| OIL1_GRP | 1.00 | 2.7540e-04 | 3.6 |
| OIL2_GRP | 2.50 | 6.9913e-05 | 9.5 |
| OIL4_GRP | 4.50 | 5.6376e-06 | 20.5 |
| OIL8_GRP | 8.50 | 1.7636e-08 | 45.5 |
| OIL16_GRP | 16.00 | 2.7172e-11 | 73.6 |
| SPARK_SPARSE | 2.00 | 3.9506e-04 | 2.0 |
| SPARK_SPARSE_GRP | 2.00 | 3.9506e-04 | 2.0 |
| SPARK_Q0 | 1.50 | 2.4673e-04 | 4.0 |
| SPARK_Q0_GRP | 1.50 | 1.7966e-04 | 5.4 |
| OIL4_CW | 4.00 | 2.4746e-06 | 24.0 |
| OIL_MIX@2bpw | 2.00 | 3.9506e-04 | 2.0 |
| OIL_MIX@3bpw | 3.00 | 1.9041e-04 | 5.2 |
| OIL_MIX@4bpw | 4.00 | 1.4796e-05 | 16.3 |
...
|---|---|---|---|
| OIL1 | 1.00 | 8.1200e-02 | |
| OIL1_GRP | 1.00 | 7.2681e-02 | yes |
| SPARK_Q0 | 1.50 | 6.8451e-02 | |
| SPARK_Q0_GRP | 1.50 | 5.3871e-02 | yes |
| OIL2 | 2.00 | 2.9860e-02 | yes |
| SPARK_SPARSE | 2.00 | 3.2018e-02 | |
| SPARK_SPARSE_GRP | 2.00 | 3.2018e-02 | |
| OIL_MIX@2bpw | 2.00 | 3.2018e-02 | |
| OIL2_GRP | 2.50 | 2.3392e-02 | yes |
| OIL_MIX@3bpw | 3.00 | 2.0832e-02 | yes |
| OIL4 | 4.00 | 1.0418e-02 | |
| OIL4_CW | 4.00 | 3.0052e-04 | yes |
| OIL_MIX@4bpw | 4.00 | 1.0418e-02 | |
| OIL4_GRP | 4.50 | 6.9698e-03 | yes |
| OIL8 | 8.00 | 1.0741e-03 | yes |
| OIL8_GRP | 8.50 | 4.0430e-06 | yes |
| OIL16 | 16.00 | 3.7384e-09 | yes |
| OIL16_GRP | 16.00 | 3.7384e-09 | yes |
| OIL32 | 32.00 | 0.0000e+00 | |

The key findings, locked:

1. SPARK_SPARSE_GRP at 2.0 BPW delivers the best quality-per-bit on sparse weight distributions (pinned Lloyd-Max plus exact zero preservation).
2. OIL8 at 8.0 BPW dominates raw quality on every distribution.
3. OIL_MIX routes OIL8 to salient blocks and low-bit formats to the bulk — the best quality per byte at a fixed target bit budget.
4. Real neural weights are sparse, and OIL's codebook quantization excels on sparse data — the format family is matched to the data the engine actually stores.
...
The four training tiers: OIL1_STE (block mean) at 1.0 BPW; SPARK_Q0_STE (sign plus learnable scale) at 1.5 BPW; OIL2_STE (pinned Lloyd-Max) at 2.0 BPW; OIL4_STE (pinned Lloyd-Max) at 4.0 BPW.

| Format | BPW | Eval MSE | vs FP32 |
|---|---|---|---|
| FP32 | 32.00 | 1.5506e-03 | baseline |
| OIL2_STE | 2.00 | 6.2237e-03 | 301% worse than FP32 |
| OIL4_STE | 4.00 | 1.2291e-03 | 21% better than FP32 |
| SPARK_Q0_STE | 1.50 | 2.3183e-03 | 49% worse than FP32 |
| OIL1_STE | 1.00 | 5.6560e-03 | 264% worse than FP32 |

The result that matters: OIL4 at 4 bits trained natively with STE beats the full-precision FP32 baseline by 21% — a quantized model, trained as quantized, outperforming the float model on the same task. The learnable parameters of SPARK_Q0 and OIL2 (per-block scales, codebooks) adapt during training; OIL1's block means are trained as Lloyd-style centroids. The quantization is not a post-hoc compression; it is the training itself. This is the measurable foundation for the claim that low-precision does not have to mean low-accuracy — it depends on how the quantization is trained.
---
...
Session 5 — The ASI Vision. DeepSeek wants AGI; ORIGIN wants ASI; the four components of the self-evolving distributed architecture; the prototype simulation (Delhi and Mumbai nodes); the global reasoning repository. Outcome: the four-component architecture.
...
Session 8 — The Diseases Return. Hypernetwork hallucination; ICL matrix memorization; the ASI-level cure; in-context activation constraints; the deterministic validation layer. Outcome: the projection and gate mechanics.
...
Session 12 — The Underestimate and the Quantum Swapper. Floating-point chaos; dangling quantization states; the zero-overhead quantum swapper; pointer swap at design-target latency. Outcome: the swapper design.
...
Session 14 — The Fourteen-Year Truth. One person, fourteen years, 100K LOC; the virtual layer pages; the verification token; memory-leak prevention. Outcome: the virtual page directory.

Session 15 — The Engine That Writes Its Own Weights. The meta-weights hypernetwork engine; the variance-control bound; the gating matrices; zero forgetting and zero leak. Outcome: the weight-writer design.
...
Session 22 — The Ultimate Masterstroke. The Balancer and the Destroyer; the thanks factor; the collapse; the forced pay loop; the ASI arrival. Outcome: the two-model strategy.
...
- Gaussian: OIL1 3.8910e-04 MSE; OIL4 9.4696e-06; OIL4_CW 1.5837e-06; OIL8_GRP 1.1254e-08; OIL16 1.7240e-11; OIL32 0.
- Sparse_90: OIL4_CW 3.5871e-04; OIL8_GRP 8.4515e-06; SPARK_SPARSE_GRP 1.8138e-03.
- Sparse_95: OIL4_CW 1.4679e-04; OIL8_GRP 4.2876e-06; SPARK_SPARSE_GRP 8.1230e-07.
- Sparse_99: OIL4_CW 5.6891e-06; OIL8_GRP 4.1486e-07; SPARK_SPARSE_GRP 2.9690e-08.
- FFN-down: OIL4 1.4796e-05; OIL4_CW 2.4746e-06; OIL8_GRP 1.7636e-08.
- Grand summary: OIL4_CW best at 4.0 tier (3.0052e-04); OIL8 best at 8.0 tier (1.0741e-03); OIL16/16_GRP 3.7384e-09.
- STE training: FP32 1.5506e-03; OIL4_STE 1.2291e-03 (21% better); SPARK_Q0_STE 2.3183e-03 (49% worse); OIL2_STE 6.2237e-03 (301% worse); OIL1_STE 5.6560e-03 (264% worse).
...
Q9. What is the difference between AGI and ASI in the project? A: DeepSeek wants AGI; ORIGIN wants ASI — a self-evolving distributed system that invents its own tools and reasoning.
...
Q19. What is the honest limit of the impossible list? A: "48T train on 14GB RAM" — impossible; "better than GPT-4 at 100x smaller" — no, scaling laws are real.
...
The library below contains the complete set: the system architecture, the triple-loop verification, the hot-swap sequence, the virtual pages, the ASI distributed system, the market strategy, the weight-writer loop, the delta upstream, the verification pipeline, the hardware tiers, the roadmap timeline, and the document structure itself.
...
## J.6. The ASI Distributed System
...
graph TD
    ...
    P3["PART THREE - The ASI Vision<br/>(Chapters 26-32)"]
    ...
    A3["Appendix C: ASI Vision Deep Dive"]
...
The honest flags are the project's public impossible list, and they deserve a full appendix because they are the rarest artifact in the industry: a company publishing its own limits.

Flag 1: "48T train on 14GB RAM" — impossible. The memory mathematics do not allow it; a claim to the contrary is a lie, and the project does not lie.

Flag 2: "Better than GPT-4 at 100x smaller" — no. Scaling laws are real; a smaller model can be more efficient, cheaper, and faster, but raw capability scales with effective compute. The engine's claims are about guarantees, not about magical scaling.

The revision protocol: if the engine grows enough that a flag becomes false, the flag is revised publicly, with the evidence that changed it, in the same repository, in the same commit as the code change. No flag is revised silently; a silent revision would be a lie.
...
Version 0.1.00 — Internal alpha release (2026-07-20). Initial OIL format prototypes (OIL2, OIL4, OIL8), basic transformer model scaffolding, tensor and memory management foundations, a math library with scalar and SIMD paths, and the project structure and build system. The reference libraries (`.llama/` and `.bitnet/`) were present but never linked — the project does not vendor competitors' code; it references for study.

Version 0.1.01 — Initial public release (2026-07-24). The core OIL format system shipped: OIL2, OIL4, OIL8, OIL16, OIL32; SPARK_Q0, SPARK_SPARSE, Binary and Ternary formats; the GRP grouped variants (OIL2_GRP, OIL4_GRP, SPARK_SPARSE_GRP); the Lloyd-Max vector quantization codebook system; sub-block grouping for lossless quantization at low bits per weight. The compute backends: Vulkan (dynamically loaded, no SDK required) and DirectX 12 on Windows; the AVX2/SIMD kernel library. The model side: a transformer architecture with flash attention, a KV cache with the OIL4 quantized variant, an autograd engine, a BPE tokenizer with Unicode support, dense and MoE trainers with vision, audio, embeddings, OCR, video, and text modules, MoE variants with expert parallelism, distributed training with tensor parallelism, FSDP and DDP, RingAllReduce and ParameterServer, an inference engine with sampler and generator, quantization and conversion CLI tools, a benchmark suite, a hardware probe, a production inference engine with streaming, and the OIL quantize and codec engines. The claims ledger tracked 47 claims, 46 proven and one pending.

The 0.1.01 release also recorded two critical benchmark corrections: OIL8 per-block k-means now beats Q8_0 by 1.02x (previously 1.4x worse with global k-means), and OIL4 per-block Lloyd-Max now beats Q4_0 by 1.11x (previously 1.6x worse). The lesson locked in the changelog: quantization quality is a property of the block statistics, not the format name.
...
- MoE advanced support versions 1 and 2, MoMMoE blocks for Mixture-of-Moments, native OIL MoE integration, and expert parallelism.
...
- CLI tools: oil_format_list for listing all registered formats, oil_evaluate for model evaluation.
- Benchmarks: bench_speed, bench_multimodal, bench_awq_gptq, bench_gpt2_inference.
- Code coverage support with the OIL_COVERAGE CMake option.
...
The removals of 0.1.02, itemized: LoRA, QLoRA, and DoRA adapter support removed from the adapter edition; Trimix (3-mix) format implementations removed with zero references remaining; orphan test files removed under the tests-not-shipped constraint; the GGUF and Safetensors import adapters removed.

The changes of 0.1.02: the adapter edition refactored from a LoRA/QLoRA/DoRA host into a native format converter (industrial formats to OIL) and a native trainer; the format registry expanded to 12 single formats, 13 two-mix variants, and 4 four-mix variants — 29 total; the build system at 82 targets (25 libraries, 25 executables, 32 tests); the claims ledger at 47 total, 46 proven and one pending.
...
| Test | Status |
|---|---|
| OILIdx write and read | PASSED |
| SHA256 corrupt detection (one byte) | PASSED |
| InNovaIDX magic header | PASSED |
| Truncated idx file detection | PASSED |
| OIL writer SHA256 dedup | PASSED |

Summary: 13 total tests, 13 passed, 0 failed, verdict PASSED.

The implementation locations, locked: SHA256 hash indexing in src/oil_format.cpp; the InNovaIDX magic header in the index writer; fail-fast corrupt detection with tensor name in the index reader; content-addressed dedup via SHA256 in the writer's dedup path; the test file tests/test_sha256_corrupt.cpp.
...
The changelog records the two corrections where the formats initially lost to industrial baselines and were fixed to win: OIL8 per-block k-means from 1.4x worse to 1.02x better than Q8_0; OIL4 per-block Lloyd-Max from 1.6x worse to 1.11x better than Q4_0. The record matters twice: it proves the formats now beat the industrial baselines, and it proves the project publishes its losses as well as its wins.
...
The training stack at 0.1.02 is the foundation of the weight-writer loop: ContinualTrainer for incremental learning without full retraining; MTPHeadTrainer for multi-token prediction; the ZeRO optimizer at three stages with offload; pipeline parallelism; HPO and NAS; the RL trainer with reward modeling; MoE support with expert parallelism and Mixture-of-Moments blocks; multimodal cross-attention and fusion; iGPU zero-copy training via Vulkan unified memory; and out-of-core training via the mmap data loader. Every capability maps to a chapter of the ASI vision: incremental learning to the additive pages; RL trainer to the reward design; MoE to the expert allocation; mmap to the virtual layer pages.
...
- The OIL format research series: bitnet-b1.58 analysis; bitnetcpp-lossless analysis; rate-distortion bounds; VQ-VAE residual coding; STE training in format; dynamic routing with gated attention mixtures; the OIL8-256-centroids study; the final verdict document.
- The system research: oil_idx research; distributed research; ASI papers; actionable improvements.
...
| Appendix H (benchmark record) | bench files 01-04, OIL vs Q8_0/Q4_0 records |
...
## J.4. The OIL Format Naming and Relationship

```mermaid
graph TD
    OIL["OIL Format Family"]
    OIL --> O1["OIL1 (block mean)"]
    OIL --> O2["OIL2 (pinned Lloyd-Max)"]
    OIL --> O4["OIL4 (pinned Lloyd-Max)"]
    OIL --> O8["OIL8 (pinned Lloyd-Max)"]
    OIL --> O16["OIL16"]
    OIL --> O32["OIL32 (lossless)"]
    OIL --> GRP["GRP grouped variants"]
    OIL --> MIX["OIL_MIX routes bits by salience"]
    OIL --> CW["OIL4_CW column-wise variant"]

    SPARK["SPARK Formats"]
    SPARK --> S0["SPARK_Q0 (sign + learnable scale)"]
    SPARK --> SS["SPARK_SPARSE (exact zeros preserved)"]
    SPARK --> SG["SPARK_SPARSE_GRP"]
```
...
The honesty principle is the oldest rule of the project, and it deserves its final restatement. The engine reports where it cannot do what is claimed. Overpromising is the only unforgivable engineering sin. The principle manifests in every layer: the honest flags at the product level, the claims ledger at the repository level, the correction records in the changelog, the pending question lists in the roadmap, and the "impossible" list that no competitor publishes. A reader who trusts the engine trusts it because the engine is the only system in the market whose documentation includes its own failures.
...
"The excess of anything is bad" — the balance philosophy shaped every decision: the frozen core and the additive experts in proportion; the Balancer and the Destroyer as a pair; the aggression tempered by the honest flags; the ambition of ASI bounded by the verification pipeline; the scale of DeepSeek and Kimi balanced by the guarantees of the engine. The engine is not the biggest model, not the cheapest model, and not the fastest model; it is the balanced model, and balance is the only position that can hold all three guarantees at once.
...
| 2 | "Zero-overhead Quantum Swapper" | Design target | The pointer-swap subsystem exists in design; "quantum" is a name, not a physics claim. |
...
| 10 | "48T train on 14GB RAM" | Honest flag — impossible | Listed in the honest flags; a claim to the contrary would be a lie. |
| 11 | "512+ tok/s on any hardware" | Honest flag — impossible | Listed in the honest flags; weak hardware with a large model will be much slower. |
...
- Part Three — The ASI Vision (Chapters 26-32)
...
|---|---|---|
| 📦 What is OIL? | Chapter 33 (framework discovery), Appendix H (benchmarks) | The OIL/SPARK format family is the numeric backbone of the narrative |
...
- AGI — Artificial General Intelligence: a hypothetical AI matching human capability across cognitive tasks.
- AI slop — machine-generated internet content that pollutes training corpora.
- ASI — Artificial Superintelligence: the stated ambition of ORIGIN, pursued as a controlled, verifiable system.
...
- Honest flag — a published limit; the engine's public "impossible list".
...
- OIL — Optimized Inference &amp; Learning; the engine's mixed-precision binary format.
...
- SFT — supervised fine-tuning: learning patterns from curated examples.
- SPARK — the engine's sparse/low-bit format family.
...
- Q: Is "quantum" in the swapper name a physics claim? A: No. It records the engineering ambition of a discrete, near-instant swap.
- Q: Can the engine really train on 14GB of RAM? A: Small models (0.1B-0.4B) yes; 48T on 14GB is on the honest-flag impossible list.
- Q: Does the engine beat GPT-4 at 100x smaller? A: No — that is on the honest-flag impossible list; scaling laws are real.
...
- Q: How do I convert a model to OIL? A: build/tools/oil-convert --input model.safetensors --output model.oil --target-bpw 1.50.
- Q: How do I run inference? A: build/tools/oil-infer --model model.oil --prompt "Hello" --max-tokens 256.
- Q: How do I train from scratch? A: build/tools/oil-train --config config.json --data data/tinyshakespeare.txt --output trained.oil.
- Q: What is the OIL format? A: A mixed-precision binary container; every weight block gets a format matched to its importance.
...
- oil-convert — convert GGUF/HF/FP32 weights to .oil
- oil-train — train a model from scratch with a config file
- oil-infer — interactive inference / generation
- oil-finetune — native OIL-format fine-tuning
- oil-info — inspect .oil file contents
- oil-bench — run the benchmark suite
...
- Honest flags: published, including the impossible list.
...
The complete, current registry of published limits. These are the claims the project promises not to make:

- ❌ "512+ tok/s on any hardware" — impossible; hardware limits are real.
- ❌ "48T train on 14GB RAM" — impossible; a 48T parameter model cannot train on 14GB.
- ❌ "Beats GPT-4 at 100x smaller" — impossible; scaling laws are real.
- ❌ "Trains billions/task in minutes" — impossible; training large models is costly.
- ❌ "Runs on any CPU with no GPU" — impossible; large models need compute.
- ❌ "Instantly ships 1000 tokens/s" — impossible; token throughput is bounded.
- ❌ "0% hallucination everywhere" — reframed: gated outputs only.
- ❌ "0% catastrophic forgetting, universally proven" — reframed: by-construction for additive pages.
- ✅ "0ms hot-swap" — reframed to "design target", pending measurement.
- ✅ "Quantum swapper" — reframed to aspirational name, not a physics claim.
- ✅ "10x processing" — reframed to unverified target.
