...
//                                   (OIL-Rank): per-layer LOW-RANK delta
//                                   ΔW ≈ B·A trained with autograd and
//                                   stored quantized in OIL/SPARK formats.
...
struct SelectiveTunerConfig {
    ...
    // STE roundtrip target: OIL16 (FP16-class) by default. Coarser lattice
    // formats (e.g. OIL8) quantize every selected block with ONE rms scale,
    ...
    Format target_format = Format::OIL16;
    bool ste_roundtrip = true;       // quantize selected blocks each step (STE)
    std::string output_path = "finetuned_selective.oil";
};
...
public:
    ...
    // Quantized OIL save of the updated model.
...
// ============================================================================
// METHOD 2 — InNova-native low-rank delta adapters ("OIL-Rank")
// ============================================================================
struct RankAdapterConfig {
    ...
    int warmup_steps = 10;
    Format factor_format = Format::OIL8; // deployment quantization of A and B
