...
struct QuantizedWeight {
    ...
    std::vector<int8_t>  indices_spark;
...
static QuantizedWeight quantize_weight(const float* src, int64_t n) {
    ...
    // OIL8: per-block 256-entry Lloyd-Max
    ...
    // OIL4: per-block 16-entry Lloyd-Max
    ...
    q.codebook_4.resize(16);
    {
        for (int64_t b = 0; b < n; b += BLOCK4) {
            ...
            cb.train(src + b, (size_t)bsz);
            for (int i = 0; i < 16; i++)
                q.codebook_4[i] = CodebookOIL4::half_to_float(cb.centroids[i]);
            for (int64_t i = 0; i < bsz; i++) {
    ...
    q.indices_spark.resize(n);
    for (int64_t i = 0; i < n; i++) {
        float v = src[i] / q.spark_scale;
        q.indices_spark[i] = (v > 0.33f) ? 1 : ((v < -0.33f) ? -1 : 0);
    }

    return q;
}

static void dequantize_oil8(const QuantizedWeight& q, float* dst, int64_t n) {
    for (int64_t i = 0; i < n; i++)
        dst[i] = q.codebook_8[q.indices_8bit[i]];
}

static void dequantize_oil4(const QuantizedWeight& q, float* dst, int64_t n) {
    for (int64_t i = 0; i < n; i++) {
...
static void dequantize_spark(const QuantizedWeight& q, float* dst, int64_t n) {
    for (int64_t i = 0; i < n; i++)
        dst[i] = (float)q.indices_spark[i] * q.spark_scale;
}
