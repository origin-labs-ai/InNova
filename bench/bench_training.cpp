...
static int64_t count_params(oil::DenseModel& model) {
    return model.param_count();
}

static void bench_forward_throughput() {
    std::cout << "\n=== Forward Throughput ===\n";
    std::vector<oil::DenseModel> models;
...
static void bench_backward_throughput() {
    ...
    oil::Tensor input({batch_size, seq_len}, oil::DType::F32);
    oil::Tensor pos({batch_size, seq_len}, oil::DType::F32);
    float* id = input.data<float>();
    float* pd = pos.data<float>();
    ...
    oil::Tensor labels({batch_size * seq_len}, oil::DType::F32);
...
static void bench_batch_size_scaling() {
    ...
    for (int bs : batch_sizes) {
        ...
        oil::Tensor input({bs, seq_len}, oil::DType::F32);
        oil::Tensor pos({bs, seq_len}, oil::DType::F32);
        float* id = input.data<float>();
        float* pd = pos.data<float>();
        for (int64_t i = 0; i < total_tokens; i++) {
            id[i] = (float)(i % cfg.vocab_size);
...
static void bench_gradient_compute() {
    ...
    oil::Tensor labels({total_tokens}, oil::DType::F32);
    float* ld = labels.data<float>();
    for (int64_t i = 0; i < total_tokens; i++)
        ld[i] = (float)((i + 1) % cfg.vocab_size);
...
int main() {
    std::cout << "=== OIL Training Benchmarks ===\n";
