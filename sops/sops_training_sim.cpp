...
int main() {
    ...
    printf("  Quantized weight training — OIL4 vs FP32\n");
    printf("================================================================\n\n");
    ...
    // ── OIL4 vs FP32 comparison ──────────────────────────────────────

    printf("\n================================================================\n");
    printf("  OIL4 vs FP32 TRAINING COMPARISON\n");
    printf("================================================================\n\n");

    StepResult fp32 = results[NUM_SIM_FORMATS - 1];
    StepResult oil4 = results[4];

    double mem_ratio = fp32.weight_memory_bytes / oil4.weight_memory_bytes;
    double ops_ratio = oil4.info_ops / fp32.info_ops;
    double time_ratio = fp32.total_time_ms / oil4.total_time_ms;
    double bw_ratio = oil4.memory_bw_gbs / (fp32.memory_bw_gbs > 0 ? fp32.memory_bw_gbs : 1.0);
    double sops_ratio = oil4.simulated_sops / (fp32.simulated_sops > 0 ? fp32.simulated_sops : 1.0);

    printf("  Metric              FP32            OIL4            Ratio\n");
    printf("  ----                ----            ----            -----\n");

    printf("  Weight memory       %.0f MB          %.0f MB          %.1fx less\n",
           fp32.weight_memory_bytes / 1e6, oil4.weight_memory_bytes / 1e6, mem_ratio);

    printf("  Info ops/step       %.0f            %.0f            %.1fx\n",
           fp32.info_ops, oil4.info_ops, ops_ratio);
    printf("  Memory BW used      %.2f GB/s       %.2f GB/s       %.1fx less\n",
           fp32.memory_bw_gbs, oil4.memory_bw_gbs,
           fp32.memory_bw_gbs / (oil4.memory_bw_gbs > 0 ? oil4.memory_bw_gbs : 1.0));
    printf("  Step time           %.2f ms         %.2f ms         %.1fx faster\n",
           fp32.total_time_ms, oil4.total_time_ms, time_ratio);
    printf("  SOPS                %.4e   %.4e   %.1fx higher\n",
           fp32.simulated_sops, oil4.simulated_sops, sops_ratio);
    ...
    printf("  FP32 SOPS:             %.4e\n", fp32.simulated_sops);
    printf("  OIL4 speedup:          %.1fx\n", time_ratio);
    printf("  OIL4 memory savings:   %.1fx\n\n", mem_ratio);

    printf("  OIL4 training is %.1fx faster with %.1fx less memory.\n", time_ratio, mem_ratio);
    printf("  Lower-bit formats (SPARK, OIL1) give even higher SOPS\n");
