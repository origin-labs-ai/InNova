...
float NTXentLoss::forward(const Tensor& emb1, const Tensor& emb2) const {
    ...
    QUANT_CHECK(B > 0, "NTXentLoss: batch must be > 0");
    QUANT_CHECK(emb1.dim(0) == emb2.dim(0) && emb1.dim(1) == emb2.dim(1),
...
Tensor CrossEncoder::forward(const Tensor& query_emb, const Tensor& doc_emb,
    ...
    QUANT_CHECK(doc_emb.dim(0) == B && doc_emb.dim(1) == S && doc_emb.dim(2) == D,
