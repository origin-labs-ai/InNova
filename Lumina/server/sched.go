...
func (s *Scheduler) load(req *LlmRequest, systemInfo ml.SystemInfo, gpus []ml.DeviceInfo, requireFull bool) bool {
	...
	if llama == nil {
		var err error
		if !req.model.IsMLX() {
			// Enforce .oil-only: only OIL format models are supported for inference
			if !llm.IsOilFile(req.model.ModelPath) {
				err = fmt.Errorf("only .oil format models are supported; rejected non-OIL model: %s", req.model.ModelPath)
				slog.Error("non-OIL model rejected", "model", req.model.ModelPath)
				req.errCh <- err
				s.loadedMu.Unlock()
				return false
			}

			slog.Info("detected OIL model, using oil-server", "model", req.model.ModelPath)
			launchOpts = req.opts
			llama, err = llm.NewOilServer(loadGpus, req.model.ModelPath, launchOpts, numParallel)
			if err != nil {
				slog.Error("failed to create oil-server", "model", req.model.ShortName, "error", err)
				req.errCh <- err
				s.loadedMu.Unlock()
				return false
			}
