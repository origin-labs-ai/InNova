...
void gemm_tiled(float alpha, const Tensor& A, const Tensor& B,
    ...
    OIL_CHECK(B.shape().dims[0] == K, "gemm_tiled: A cols != B rows");
    OIL_CHECK(C.shape().dims[0] == M && C.shape().dims[1] == N,
...
void gemm_tiled_bias(float alpha, const Tensor& A, const Tensor& B,
                     ...
                     Transpose trans) {
    OIL_CHECK(C.shape().rank == 2, "gemm_tiled_bias: C must be 2D");
    int64_t N = C.shape().dims[1];
    OIL_CHECK(bias.numel() == N, "gemm_tiled_bias: bias length != N cols");
...
#else // !OIL_AVX2 — scalar fallbacks
