// ============================================================================
// kernel_production.cpp — Production SIMD kernels for OIL inference
// ============================================================================
#include "oil/kernel_production.h"
...
[[maybe_unused]] static inline float decode_spark(uint8_t byte, int shift) {
...
static void gemv_spark_tiled_scalar(const uint8_t* packed_w, float scale,
    ...
    for (int m_start = 0; m_start < M; m_start += tile_m) {
        ...
        for (int m = m_start; m < m_end; m++) {
            ...
            for (int k = 0; k < K; k++)
                sum += decode_spark(w_row[k / 4], k % 4) * x[k];
            y[m] = sum * scale;
        }
    }
}

static void gemv_oil4_tiled_scalar(const uint8_t* packed_indices, const uint16_t* codebook,
...
static void gemv_oil8_tiled_scalar(const uint8_t* indices, const float* codebook,
                                    float scale, const float* x, float* y,
                                    int M, int K, int tile_m) {
    for (int m_start = 0; m_start < M; m_start += tile_m) {
        int m_end = std::min(m_start + tile_m, M);
...
static void gemv_spark_tiled_avx2(const uint8_t* packed_w, float scale,
                                   const float* x, float* y,
                                   int M, int K, int tile_m) {
    int bytes_per_row = (K + 3) / 4;
    ...
    for (int i = 0; i < 256; i++)
        for (int s = 0; s < 4; s++)
            lut[i][s] = decode_spark((uint8_t)i, s);
    ...
    for (int m_start = 0; m_start < M; m_start += tile_m) {
        ...
        for (int m = m_start; m < m_end; m++) {
            ...
            // Tail
            for (; k < K; k++)
                result += decode_spark(w_row[k / 4], k % 4) * x[k];
            y[m] = result * scale;
        }
    }
}

static void gemv_oil4_tiled_avx2(const uint8_t* packed_indices, const uint16_t* codebook,
                                  float scale, const float* x, float* y,
                                  int M, int K, int tile_m) {
    float centroids[16];
    for (int i = 0; i < 16; i++) centroids[i] = fp16_to_float(codebook[i]);
...
static void gemv_oil8_tiled_avx2(const uint8_t* indices, const float* codebook,
                                  float scale, const float* x, float* y,
                                  int M, int K, int tile_m) {
    for (int m_start = 0; m_start < M; m_start += tile_m) {
        int m_end = std::min(m_start + tile_m, M);
...
void gemv_spark_tiled(const uint8_t* packed_w, float scale,
    ...
    if (isa >= 2) {
#if defined(__AVX2__)
        gemv_spark_tiled_avx2(packed_w, scale, x, y, M, K, tile_m);
        return;
#endif
    }
    gemv_spark_tiled_scalar(packed_w, scale, x, y, M, K, tile_m);
}
...
void gemv_oil4_tiled(const uint8_t* packed_indices, const uint16_t* codebook,
    ...
    if (isa >= 2) {
#if defined(__AVX2__)
        gemv_oil4_tiled_avx2(packed_indices, codebook, scale, x, y, M, K, tile_m);
        return;
#endif
    }
    gemv_oil4_tiled_scalar(packed_indices, codebook, scale, x, y, M, K, tile_m);
}
...
void gemv_oil8_tiled(const uint8_t* indices, const float* codebook,
    ...
    if (isa >= 2) {
#if defined(__AVX2__)
        gemv_oil8_tiled_avx2(indices, codebook, scale, x, y, M, K, tile_m);
        return;
#endif
    }
    gemv_oil8_tiled_scalar(indices, codebook, scale, x, y, M, K, tile_m);
}
...
void gemv_spark_batch(const uint8_t* packed_w, float scale,
                       ...
                       int M, int N, int K) {
    for (int n = 0; n < N; n++)
        gemv_spark_tiled(packed_w, scale, X + (int64_t)n * K, Y + (int64_t)n * M, M, K);
}
...
void gemv_oil4_batch(const uint8_t* packed_indices, const uint16_t* codebook,
                     ...
                     int M, int N, int K) {
    for (int n = 0; n < N; n++)
        gemv_oil4_tiled(packed_indices, codebook, scale, X + (int64_t)n * K, Y + (int64_t)n * M, M, K);
}
...
void gemv_oil8_batch(const uint8_t* indices, const float* codebook,
                     ...
                     int M, int N, int K) {
    for (int n = 0; n < N; n++)
        gemv_oil8_tiled(indices, codebook, scale, X + (int64_t)n * K, Y + (int64_t)n * M, M, K);
}
    ...
    for (int m = 0; m < M; m++) {
        for (int k_start = 0; k_start < K; k_start += block_size) {
            ...
            for (int k = k_start; k < k_end; k++) {
                ...
                float w_val = decode_spark(((const uint8_t*)weights)[byte_idx], k % 4);
...
KernelPerf profile_gemv_spark(const uint8_t* packed_w, float scale,
    ...
    for (int r = 0; r < repeats; r++)
        gemv_spark_tiled(packed_w, scale, x, y, M, K);
...
KernelPerf profile_gemv_oil4(const uint8_t* packed_indices, const uint16_t* codebook,
    ...
    auto wall_start = std::chrono::high_resolution_clock::now();
    for (int r = 0; r < repeats; r++)
        gemv_oil4_tiled(packed_indices, codebook, scale, x, y, M, K);
    auto wall_end = std::chrono::high_resolution_clock::now();
    uint64_t end = rdtsc();
    p.cycles = (end - start) / repeats;
    double wall_sec = std::chrono::duration<double>(wall_end - wall_start).count() / repeats;
...
KernelPerf profile_gemv_oil8(const uint8_t* indices, const float* codebook,
    ...
    auto wall_start = std::chrono::high_resolution_clock::now();
    for (int r = 0; r < repeats; r++)
        gemv_oil8_tiled(indices, codebook, scale, x, y, M, K);
    auto wall_end = std::chrono::high_resolution_clock::now();
    uint64_t end = rdtsc();
    p.cycles = (end - start) / repeats;
    double wall_sec = std::chrono::duration<double>(wall_end - wall_start).count() / repeats;
