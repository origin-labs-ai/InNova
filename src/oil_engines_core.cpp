...
static void dequant_tensor_oil8_avx2(const uint8_t* indices, float* out,
...
static void dequant_tensor_oil4_avx2(const uint8_t* packed, float* out,
                                      int64_t n, const float* cb) {
    int64_t i = 0;
    for (; i + 8 <= n; i += 8) {
        uint32_t p4;
...
// AVX2 quant_gemm: OIL8 (256-entry codebook LUT)
static void quant_gemm_oil8_avx2(const float* ad, float* cd,
                                  const uint8_t* b_idx,
...
// AVX2 quant_gemm: OIL4 (packed 4-bit indices, 16-entry codebook)
static void quant_gemm_oil4_avx2(const float* ad, float* cd,
...
// AVX2 quant_gemm: SPARK add/sub only (no multiply in inner loop)
static void quant_gemm_spark_avx2(const float* ad, float* cd,
