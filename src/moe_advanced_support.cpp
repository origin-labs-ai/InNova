...
// ---- SPARK_MOE ----
float SparkMoE::load_balance_loss(const Tensor& gates) const {
    int64_t T = gates.dim(0), E = gates.dim(1);
    const float* g = gates.data<float>();
    std::vector<double> frac(E, 0.0), prob(E, 0.0);
    for (int64_t t = 0; t < T; ++t) {
    ...
    double loss = 0.0;
    for (int64_t e = 0; e < E; ++e) {
        double q_penalty = (double)(spark_scales[(size_t)e] > 0.5f ? 1 : 2);
        loss += frac[e] * prob[e] * q_penalty;
    }
    return (float)(loss * (double)E);
}

float SparkMoE::z_loss(const Tensor& logits) const {
    const float* d = logits.data<float>();
    int64_t n = logits.numel();
    float mean = 0.0f;
...
int64_t SparkMoE::compute_capacity(int64_t T) const {
    int64_t E = config.num_experts, K = config.top_k;
    return (K * T + E - 1) / E;
}

std::vector<uint8_t> SparkMoE::export_weights() const {
    int64_t nr = router.weight.numel();
    std::vector<uint8_t> d(sizeof(int64_t) + (size_t)nr * sizeof(float));
    std::memcpy(d.data(), &nr, sizeof(int64_t));
    std::memcpy(d.data() + sizeof(int64_t), router.weight.data<float>(), (size_t)nr * sizeof(float));
...
void SparkMoE::import_weights(const std::vector<uint8_t>& data) {
    int64_t nr = 0; std::memcpy(&nr, data.data(), sizeof(int64_t));
    std::memcpy(router.weight.data<float>(), data.data() + sizeof(int64_t), (size_t)nr * sizeof(float));
}

// ---- OIL1_MOE ----
float Oil1MoE::load_balance_loss(const Tensor& gates) const {
    int64_t T = gates.dim(0), E = gates.dim(1);
    const float* g = gates.data<float>();
    std::vector<double> frac(E, 0.0);
    for (int64_t t = 0; t < T; ++t) {
    ...
    for (int64_t e = 0; e < E; ++e) {
        ...
        double bin_w = (double)(oil1_scales[(size_t)e] > 0.0f ? 1 : 3);
        loss += (f - 1.0 / (double)E) * (f - 1.0 / (double)E) * bin_w;
    }
    return (float)(loss * (double)E * (double)E);
}

float Oil1MoE::z_loss(const Tensor& logits) const {
    const float* d = logits.data<float>();
    int64_t n = logits.numel();
    float sum_sq = 0.0f;
...
int64_t Oil1MoE::compute_capacity(int64_t T) const {
    int64_t E = config.num_experts, K = config.top_k;
    return (K * T + E - 1) / E;
}

std::vector<uint8_t> Oil1MoE::export_weights() const {
    int64_t nr = router.weight.numel();
    std::vector<uint8_t> d(sizeof(int64_t) + (size_t)nr * sizeof(float));
    std::memcpy(d.data(), &nr, sizeof(int64_t));
    std::memcpy(d.data() + sizeof(int64_t), router.weight.data<float>(), (size_t)nr * sizeof(float));
...
void Oil1MoE::import_weights(const std::vector<uint8_t>& data) {
    int64_t nr = 0; std::memcpy(&nr, data.data(), sizeof(int64_t));
    std::memcpy(router.weight.data<float>(), data.data() + sizeof(int64_t), (size_t)nr * sizeof(float));
}

// ---- OIL8_MOE ----
float OIL8MoE::load_balance_loss(const Tensor& gates) const {
    int64_t T = gates.dim(0), E = gates.dim(1);
    const float* g = gates.data<float>();
    std::vector<double> frac(E, 0.0);
    for (int64_t t = 0; t < T; ++t) {
...
float OIL8MoE::z_loss(const Tensor& logits) const {
    const float* d = logits.data<float>();
    int64_t n = logits.numel();
    float maxv = d[0];
    for (int64_t i = 1; i < n; ++i) if (d[i] > maxv) maxv = d[i];
...
int64_t OIL8MoE::compute_capacity(int64_t T) const {
    int64_t E = config.num_experts, K = config.top_k;
    return (K * T + E - 1) / E;
}

std::vector<uint8_t> OIL8MoE::export_weights() const {
    int64_t nr = router.weight.numel();
    std::vector<uint8_t> d(sizeof(int64_t) + (size_t)nr * sizeof(float));
    std::memcpy(d.data(), &nr, sizeof(int64_t));
    std::memcpy(d.data() + sizeof(int64_t), router.weight.data<float>(), (size_t)nr * sizeof(float));
...
void OIL8MoE::import_weights(const std::vector<uint8_t>& data) {
    int64_t nr = 0; std::memcpy(&nr, data.data(), sizeof(int64_t));
    std::memcpy(router.weight.data<float>(), data.data() + sizeof(int64_t), (size_t)nr * sizeof(float));
}

// ---- OIL4_MOE ----
float OIL4MoE::load_balance_loss(const Tensor& gates) const {
    int64_t T = gates.dim(0), E = gates.dim(1);
    const float* g = gates.data<float>();
    std::vector<double> frac(E, 0.0), prob(E, 0.0);
    for (int64_t t = 0; t < T; ++t) {
...
float OIL4MoE::z_loss(const Tensor& logits) const {
    const float* d = logits.data<float>();
    int64_t n = logits.numel();
...
int64_t OIL4MoE::compute_capacity(int64_t T) const {
    int64_t E = config.num_experts, K = config.top_k;
    return (K * T + E - 1) / E;
}

std::vector<uint8_t> OIL4MoE::export_weights() const {
    int64_t nr = router.weight.numel();
    std::vector<uint8_t> d(sizeof(int64_t) + (size_t)nr * sizeof(float));
    std::memcpy(d.data(), &nr, sizeof(int64_t));
    std::memcpy(d.data() + sizeof(int64_t), router.weight.data<float>(), (size_t)nr * sizeof(float));
...
void OIL4MoE::import_weights(const std::vector<uint8_t>& data) {
    int64_t nr = 0; std::memcpy(&nr, data.data(), sizeof(int64_t));
    std::memcpy(router.weight.data<float>(), data.data() + sizeof(int64_t), (size_t)nr * sizeof(float));
}
