// ============================================================================
// PILLAR 3: VectorQuantizer — EMA-based VQ + OIL8/OIL4 Codebooks
// ============================================================================

#include "oil/vector_quantizer.h"
#include <cmath>
#include <algorithm>
#include <numeric>
#include <cstring>
...
// ============================================================================
// OIL8VectorQuantizer — 256-entry FP32 codebook
// ============================================================================

OIL8VectorQuantizer::OIL8VectorQuantizer() {
...
OIL8VectorQuantizer::OIL8VectorQuantizer(const std::vector<float>& initial_codebook)
    : codebook_(initial_codebook) {
    codebook_sum_.resize(256);
    codebook_counts_.resize(256, 0);
}

int OIL8VectorQuantizer::find_nearest(float value) const {
    float min_dist = 1e30f;
    int best = 0;
    for (int i = 0; i < 256; ++i) {
...
std::vector<uint8_t> OIL8VectorQuantizer::quantize(const float* weights, int64_t n) {
...
void OIL8VectorQuantizer::dequantize(const uint8_t* indices, float* output, int64_t n) const {
    for (int64_t i = 0; i < n; ++i) {
        output[i] = codebook_[indices[i]];
    }
}

void OIL8VectorQuantizer::update_ema(const float* weights, const uint8_t* indices, int64_t n) {
...
// ============================================================================
// OIL4VectorQuantizer — 16-entry FP16 codebook
// ============================================================================

OIL4VectorQuantizer::OIL4VectorQuantizer() {
    codebook_.resize(16);
...
OIL4VectorQuantizer::OIL4VectorQuantizer(const std::vector<uint16_t>& initial_codebook)
    : codebook_(initial_codebook) {
    codebook_sum_.resize(16);
    codebook_counts_.resize(16, 0);
}

float OIL4VectorQuantizer::fp16_to_float(uint16_t h) {
...
uint16_t OIL4VectorQuantizer::float_to_fp16(float f) {
    uint32_t bits;
...
int OIL4VectorQuantizer::find_nearest(float value) const {
    float min_dist = 1e30f;
    int best = 0;
    for (int i = 0; i < 16; ++i) {
...
std::vector<uint8_t> OIL4VectorQuantizer::quantize(const float* weights, int64_t n) {
...
void OIL4VectorQuantizer::dequantize(const uint8_t* packed_indices, float* output, int64_t n) const {
    for (int64_t i = 0; i < n; ++i) {
...
void OIL4VectorQuantizer::update_ema(const float* weights, const uint8_t* packed_indices, int64_t n) {
    std::fill(codebook_sum_.begin(), codebook_sum_.end(), 0.0f);
    std::fill(codebook_counts_.begin(), codebook_counts_.end(), 0);
    for (int64_t i = 0; i < n; ++i) {
        uint8_t byte = packed_indices[i / 2];
