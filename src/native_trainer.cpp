...
NativeOILTrainer::NativeOILTrainer(DenseModel* model, const NativeTrainConfig& cfg)
    : model_(model), cfg_(cfg) {
    oil::collect_dense_params(model_, model_params_);
    total_params_ = 0;
    ...
    weight_store_ = std::make_unique<NativeOILWeightStore>(total_params_, cfg_.block_size);
...
void NativeOILTrainer::push_weights_to_model() {
...
void NativeOILTrainer::pull_gradients_from_model() {
    size_t offset = 0;
    for (auto* p : model_params_) {
        size_t n = (size_t)p->numel();
        if (p->has_grad()) {
...
void NativeOILTrainer::update_sensitivity() {
...
void NativeOILTrainer::allocate_formats() {
...
void NativeOILTrainer::apply_two_timescale_sgd() {
    ...
    for (size_t b = 0; b < num_blocks; b++) {
        ...
        for (size_t i = start; i < end; i++) {
            ...
            switch (fmt) {
                case NativeFormat::OIL1: cv = oil1_value(idx); break;
                case NativeFormat::OIL4:    cv = NativeOILWeightStore::global_oil4_codebook().centroid(idx); break;
                case NativeFormat::OIL8:    cv = NativeOILWeightStore::global_codebook().centroid(idx); break;
                default: cv = 0.0f;
            }
    ...
    // Pass lr_scale=0 so apply_oil_update skips its own scale update
    weight_store_->apply_oil_update(grad_buffer_.get(), 0.0f, lr_w);
}

NativeTrainMetrics NativeOILTrainer::train_step(const float* input, const float* target,
    ...
    size_t frozen = 0, oil8 = 0, spark = 0, oil1 = 0;
    double avg_s = 0.0;
    for (size_t i = 0; i < total_params_; i++) {
        NativeFormat fmt = weight_store_->get_format(i);
        if (fmt == NativeFormat::OIL8) oil8++;
        else if (fmt == NativeFormat::OIL1) oil1++;
        else spark++;
    ...
    metrics.oil8_count = oil8;
    metrics.oil1_count = oil1;
    metrics.spark_count = spark;
    metrics.avg_scale = avg_s / (double)total_params_;
    return metrics;
}

void NativeOILTrainer::warmup_phase(const std::vector<std::vector<float>>& data) {
    if (data.empty()) return;
    std::cout << "[NativeOIL] Warmup: " << cfg_.warmup_steps << " FP32 steps\n";
    size_t offset = 0;
    for (auto* p : model_params_) {
        size_t n = (size_t)p->numel();
    ...
    weight_store_->initialize(temp_weight_buffer_.get(), sensitivity_.get(),
                               cfg_.frac_oil8, cfg_.frac_spark);
    ...
    allocate_formats();
    std::cout << "[NativeOIL] CID: OIL8=" << (cfg_.frac_oil8 * 100.0f)
              << "% T=" << (cfg_.frac_spark * 100.0f)
              << "% B=" << ((1.0f - cfg_.frac_oil8 - cfg_.frac_spark) * 100.0f) << "%\n";
}

void NativeOILTrainer::train(const std::vector<std::vector<float>>& train_data,
                              const std::vector<std::vector<float>>& eval_data) {
    if (train_data.empty()) return;
    warmup_phase(train_data);
    std::cout << "\n[NativeOIL] Training (" << cfg_.max_steps << " steps)\n";
    for (size_t step = 0; step < cfg_.max_steps; step++) {
        ...
        if ((step + 1) % cfg_.log_interval == 0) {
            std::cout << "Step " << (step + 1) << "/" << cfg_.max_steps
                      ...
                      << " O8=" << m.oil8_count << " SP=" << m.spark_count
                      << " O1=" << m.oil1_count << " s=" << m.avg_scale << "\n";
        }
        ...
        if ((step + 1) % cfg_.save_interval == 0) {
            save_checkpoint(cfg_.output_dir + "/native_oil_step_" + std::to_string(step + 1) + ".oil");
        }
    }
}

double NativeOILTrainer::evaluate(const std::vector<std::vector<float>>& eval_data) {
...
void NativeOILTrainer::save_checkpoint(const std::string& path) {
...
void NativeOILTrainer::load_checkpoint(const std::string& path) {
