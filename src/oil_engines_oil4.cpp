...
void OIL4Engine::train_codebook_per_block(const float* data, int64_t n,
...
void OIL4Engine::quantize_per_block(const float* data, int64_t n, int64_t block_size,
...
void OIL4Engine::dequantize_per_block(const uint8_t* indices, const float* scales,
...
#ifdef OIL_HAS_AVX2
    if (n >= 8) {
        dequant_tensor_oil4_avx2(indices, od, n, codebook_.data());
        return out;
    }
#endif
    for (int64_t i = 0; i < n; ++i) {
...
// ===========================================================================
// OIL4 Engine: Extensions
// ===========================================================================

Tensor OIL4Engine::quantize_tensor(const float* data, int64_t n) const {
...
Tensor OIL4Engine::quant_gemm(const Tensor& a, const uint8_t* b_idx,
...
#ifdef OIL_HAS_AVX2
    quant_gemm_oil4_avx2(ad, cd, b_idx, M, N, K, codebook_.data());
#else
...
void OIL4Engine::quantize_per_channel(const Tensor& t, int channel_dim,
                                       Tensor& q, Tensor& scales) const {
    OIL_CHECK(t.rank() == 2, "OIL4 per-channel expects 2D tensor");
...
void OIL4Engine::dequantize_per_channel(const Tensor& q, const Tensor& scales,
...
float OIL4Engine::quant_error(const Tensor& original, const Tensor& reconstructed) const {
    return compute_quant_mse(original, reconstructed);
}

float OIL4Engine::quant_snr(const Tensor& original, const Tensor& reconstructed) const {
