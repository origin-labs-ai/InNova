...
//
// quantize_block() delegates to oil::quantize_block_all (oil/block_codec.h):
// the single source of truth for on-disk block payloads. All 15 OIL/SPARK
...
//    the claimed BPW (e.g. OIL2_GRP @ 2.5 -> OIL4-class quality -- the native
...
Format select_tensor_format(const std::string& name, int64_t numel, Format base) {
    ...
    if (critical && numel <= 262144) return Format::OIL32;
    ...
    if (b <= 1.0f)  return Format::OIL1_GRP;
    if (b <= 1.5f)  return Format::SPARK_Q0_GRP;
    if (b <= 2.0f)  return Format::OIL2_GRP;
    if (b <= 4.0f)  return Format::OIL4_GRP;
    if (b <= 8.0f)  return Format::OIL8_GRP;
    if (b <= 16.0f) return Format::OIL16_GRP;
    return Format::OIL32;
}

// Quantize one block to `fmt`, filling codebook + indices. Every OIL/SPARK
...
std::vector<Format> allocate_tensor_formats(const std::string& name, int64_t numel,
    ...
    // Compound (TWI_MIX/QUAD/SPARK_MIX): only the critical-small-tensor guard
    ...
    if (guard == Format::OIL32) {
        for (auto& f : out) f = Format::OIL32;
        if (plan_out) fill_flat_plan(*plan_out, numel, bs, out);
        return out;
    }

    // SPARK_MIX adaptive allocator: measured benefit-per-byte greedy under a
...
// The funnel: AdapterTensors (FP32) -> .oil file
//
...
// overridden by select_tensor_format() for critical (OIL32) and embedding
