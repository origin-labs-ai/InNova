// ============================================================================
// bench_spark_loss.cpp — Measure SPARK_SPARSE_GRP vs BF16 quality loss on model
// ============================================================================
// Usage:
//   bench_spark_loss --input ./Ornith-1.0-9B [--verbose]
//
...
static Args parse_args(int argc, char** argv) {
    ...
    for (int i = 1; i < argc; i++) {
        if (strcmp(argv[i], "--input") == 0 && i + 1 < argc)
            ...
            std::printf("Usage: bench_spark_loss --input <model_dir|safetensors> [--verbose]\n");
            std::printf("Measures SPARK_SPARSE_GRP quantization MSE vs BF16/FP32 reference.\n");
            return a;
        }
    }
    return a;
...
int main(int argc, char** argv) {
    ...
    FormatDescriptor spark_fmt = FormatRegistry::parse_format_name("SPARK_SPARSE_GRP");
    if (spark_fmt.name.empty()) {
        std::fprintf(stderr, "Error: SPARK_SPARSE_GRP not found in registry\n");
        return 1;
    }
    ...
    for (const auto& shard : shard_files) {
        ...
        for (const auto& t : tensors) {
            ...
            float mse = FormatRegistry::evaluate_format_quality(t.data.data(), n, spark_fmt);
    ...
    if (total_elems > 0) {
        ...
        std::printf("SPARK_SPARSE_GRP @2.0 BPW | Expected inference loss: negligible\n");
    } else {
