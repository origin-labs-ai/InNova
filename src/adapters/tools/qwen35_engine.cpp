...
// ---- block decode ---------------------------------------------------------
// Every OIL/SPARK format (0..14) is decoded via the canonical block codec, so
// the engine runs TWI_MIX / QUAD_MIX files too (they are per-block formats).
void Qwen35Engine::decode_block(const OILReader& rd, uint32_t block_id, uint32_t nw, float* out) {
...
Qwen35Engine::Qwen35Engine()
    ...
    skip_full_ = getenv("OIL_SKIP_FULL") != nullptr;
    skip_gla_ = getenv("OIL_SKIP_GLA") != nullptr;
}
...
bool Qwen35Engine::load(const std::string& oil_path) {
    reader_ = new OILReader(oil_path);
