// ============================================================================
// oil_verify_roundtrip.cpp — verify a .oil file against the original
...
int main(int argc, char** argv) {
    if (argc < 3) {
        std::printf("usage: oil_verify_roundtrip <model_dir> <model.oil> [tensors_to_check] [name_filter]\n");
        return 1;
    }
    ...
    for (const auto& name : names) {
        ...
        int64_t n = t.numel();
        if ((int64_t)orig.size() != n) { std::printf("  %-52s SIZE MISMATCH orig=%zu oil=%lld\n", name.c_str(), orig.size(), (long long)n); continue; }
