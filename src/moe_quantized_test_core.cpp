...
bool test_moe_load_balance_loss() {
    ...
    }
    {
        ...
        MoEOutput out = moe.forward(x);
        float lb = moe.load_balance_loss(out.router_logits);
        test_lb_equality("SPARK_MOE", lb, 0.0f, 10.0f);
    }
    {
        MoEAllConfig cfg; cfg.num_experts = 8; cfg.top_k = 2; cfg.expert_hidden_size = 256;
        Oil1MoE moe(D, cfg);
        MoEOutput out = moe.forward(x);
        float lb = moe.load_balance_loss(out.router_logits);
        test_lb_equality("OIL1_MOE", lb, 0.0f, 10.0f);
    }
    {
        MoEAllConfig cfg; cfg.num_experts = 8; cfg.top_k = 2; cfg.expert_hidden_size = 256;
        OIL8MoE moe(D, cfg);
        MoEOutput out = moe.forward(x);
        float lb = moe.load_balance_loss(out.router_logits);
        test_lb_equality("OIL8_MOE", lb, 0.0f, 10.0f);
    }
    {
        MoEAllConfig cfg; cfg.num_experts = 8; cfg.top_k = 2; cfg.expert_hidden_size = 256;
        OIL4MoE moe(D, cfg);
        MoEOutput out = moe.forward(x);
        float lb = moe.load_balance_loss(out.router_logits);
        test_lb_equality("OIL4_MOE", lb, 0.0f, 10.0f);
    }
