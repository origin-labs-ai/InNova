...
// ===========================================================================
// SPARK Engine: {-1, 0, +1} with per-block scale (BitNet b1.58)
// ===========================================================================

SparkEngine::SparkEngine(int64_t block_size) : block_size_(block_size) {}

Tensor SparkEngine::quantize(const Tensor& weight) {
    ...
    for (int64_t b = 0; b < num_blocks; ++b) {
        ...
        for (int64_t i = start; i < end; ++i) {
            ...
            int8_t spark_val;
            if (normalized > 0.33f) spark_val = 1;
            else if (normalized < -0.33f) spark_val = -1;
            else spark_val = 0;
            uint8_t code = (uint8_t)((spark_val + 1) & 0x3);
...
Tensor SparkEngine::dequantize(const Tensor& packed, const Tensor& scales, int64_t n) {
    ...
    for (int64_t i = 0; i < n; ++i) {
        ...
        float spark_val = (code == 0) ? -1.0f : ((code == 2) ? 1.0f : 0.0f);
        od[i] = spark_val * scale;
    }
    return out;
}
...
Tensor SparkEngine::quantize_batch(const Tensor& t) {
    int64_t n = t.numel();
    int64_t num_blocks = (n + block_size_ - 1) / block_size_;
    int64_t packed_size = (n + 3) / 4;
    int64_t hdr = 3;
...
Tensor SparkEngine::dequantize_batch(const Tensor& q) {
...
Tensor SparkEngine::quant_gemm(const Tensor& a, const Tensor& b_packed,
    ...
    // Fallback to scalar: quant_gemm_spark_avx2 is static in another TU
...
void SparkEngine::quantize_per_channel(const Tensor& t, int channel_dim,
                                          Tensor& q, Tensor& scales) {
    OIL_CHECK(t.rank() == 2, "SPARK per-channel expects 2D tensor");
...
void SparkEngine::dequantize_per_channel(const Tensor& q, const Tensor& scales,
    ...
    for (int64_t c = 0; c < n; ++c) {
        ...
        for (int64_t i = 0; i < other; ++i) {
            ...
            uint8_t code = (pd[byte_idx] >> bit_off) & 0x3;
            float spark_val = (code == 0) ? -1.0f : ((code == 2) ? 1.0f : 0.0f);
            od[idx] = spark_val * scale;
        }
    }
}

float SparkEngine::quant_error(const Tensor& original, const Tensor& reconstructed) {
    return compute_quant_mse(original, reconstructed);
}

float SparkEngine::quant_snr(const Tensor& original, const Tensor& reconstructed) {
