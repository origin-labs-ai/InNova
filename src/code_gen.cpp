...
std::string CodeGenerator::format_type_string(Format fmt) const {
    switch (fmt) {
        case Format::OIL1:    return "int8_t";
        case Format::SPARK_Q0: return "int8_t";
        case Format::OIL4:    return "uint8_t";
        case Format::OIL8:    return "uint8_t";
        case Format::OIL16:    return "float";
        case Format::OIL32:    return "float";
...
std::string CodeGenerator::gemm_tiled(int64_t M, int64_t N, int64_t K, int tile_m, int tile_n, int tile_k, SIMDTarget simd) {
    ...
    code << "\nextern \"C\" void oil_gemm(const float* __restrict__ A,\n";
...
std::string CodeGenerator::generate_rms_norm(int64_t dim, SIMDTarget simd) {
    return norm_kernel("oil_rms_norm", dim, true, simd);
}

std::string CodeGenerator::generate_layer_norm(int64_t dim, SIMDTarget simd) {
    return norm_kernel("oil_layer_norm", dim, false, simd);
}
...
std::string CodeGenerator::generate_relu(int64_t n, SIMDTarget simd) {
    return activation_kernel("oil_relu", n, "relu", simd);
}

std::string CodeGenerator::generate_silu(int64_t n, SIMDTarget simd) {
    return activation_kernel("oil_silu", n, "silu", simd);
}

std::string CodeGenerator::generate_gelu(int64_t n, SIMDTarget simd) {
    return activation_kernel("oil_gelu", n, "gelu", simd);
}
...
std::string CodeGenerator::generate_softmax(int64_t dim, SIMDTarget simd) {
    std::ostringstream code;
    code << "#include <cstdint>\n#include <cmath>\n";
    code << "\nextern \"C\" void oil_softmax(const float* __restrict__ input,\n";
    code << "    float* __restrict__ output, int64_t n) {\n\n";
...
std::string CodeGenerator::generate_add(int64_t n, SIMDTarget simd) {
    ...
    code << "#include <cstdint>\n";
    if (simd != SIMDTarget::SCALAR) code << "#include <immintrin.h>\n";
    code << "\nextern \"C\" void oil_add(const float* __restrict__ a,\n";
...
std::string CodeGenerator::generate_mul(int64_t n, SIMDTarget simd) {
    ...
    code << "#include <cstdint>\n";
    if (simd != SIMDTarget::SCALAR) code << "#include <immintrin.h>\n";
    code << "\nextern \"C\" void oil_mul(const float* __restrict__ a,\n";
    code << "    const float* __restrict__ b, float* __restrict__ c, int64_t n) {\n";
...
std::string CodeGenerator::generate_scale(int64_t n, float scale_val, SIMDTarget simd) {
    ...
    code << "#include <cstdint>\n";
    if (simd != SIMDTarget::SCALAR) code << "#include <immintrin.h>\n";
    code << "\nextern \"C\" void oil_scale(const float* __restrict__ input,\n";
...
std::string CodeGenerator::generate_residual_add(int64_t n, SIMDTarget simd) {
    ...
    code << "#include <cstdint>\n";
    if (simd != SIMDTarget::SCALAR) code << "#include <immintrin.h>\n";
    code << "\nextern \"C\" void oil_residual_add(const float* __restrict__ a,\n";
...
std::string CodeGenerator::generate_embedding_lookup(int64_t vocab_size, int64_t embed_dim, SIMDTarget simd) {
    std::ostringstream code;
    code << "#include <cstdint>\n";
    code << "\nextern \"C\" void oil_embedding_lookup(const float* __restrict__ table,\n";
...
std::string CodeGenerator::generate_quantize_dequantize(int64_t n, Format src_format, Format dst_format) {
    std::ostringstream code;
    code << "#include <cstdint>\n#include <cmath>\n";
    code << "\nextern \"C\" void oil_quantize_dequantize(const float* __restrict__ input,\n";
    code << "    float* __restrict__ output, int64_t n) {\n\n";

    if (src_format == Format::OIL32 && dst_format == Format::OIL8) {
        code << indent(1) << "for (int64_t i = 0; i < n; i++) {\n";
        ...
        code << indent(1) << "}\n";
    } else if (src_format == Format::OIL32 && dst_format == Format::OIL4) {
        code << indent(1) << "for (int64_t i = 0; i < n; i++) {\n";
        code << indent(2) << "float v = input[i];\n";
        ...
        code << indent(1) << "}\n";
    } else if (src_format == Format::OIL32 && dst_format == Format::SPARK_Q0) {
        code << indent(1) << "for (int64_t i = 0; i < n; i++) {\n";
        code << indent(2) << "float v = input[i];\n";
        code << indent(2) << "output[i] = v > 0.33f ? 1.0f : (v < -0.33f ? -1.0f : 0.0f);\n";
        code << indent(1) << "}\n";
    } else if (src_format == Format::OIL32 && dst_format == Format::OIL1) {
        code << indent(1) << "for (int64_t i = 0; i < n; i++) {\n";
...
std::string CodeGenerator::generate_format_dispatch(const std::string& op_name, const std::vector<Format>& formats) {
    std::ostringstream code;
    code << "#include \"oil/types.h\"\n\n";
    code << "void " << op_name << "_dispatch(const void* input, void* output, int64_t n, oil::Format fmt) {\n";

    for (auto fmt : formats) {
        code << indent(1) << "if (fmt == oil::Format::" << format_name(fmt) << ") {\n";
...
std::string CodeGenerator::generate_format_branch(const std::string& op_name, Format format, int64_t dim) {
    ...
    switch (format) {
        case Format::OIL1:
            code << "void " << op_name << "_oil1(const int8_t* in, int8_t* out, int64_t n) {\n";
            code << indent(1) << "for (int64_t i = 0; i < n; i++) out[i] = in[i] ^ 1;\n";
            code << "}\n";
            break;
        case Format::SPARK_Q0:
            code << "void " << op_name << "_spark_q0(const int8_t* in, int8_t* out, int64_t n) {\n";
            code << indent(1) << "for (int64_t i = 0; i < n; i++) out[i] = -in[i];\n";
            code << "}\n";
            break;
        case Format::OIL4:
            code << "void " << op_name << "_oil4(const uint8_t* in, float* out, int64_t n) {\n";
            code << indent(1) << "for (int64_t i = 0; i < n; i++) {\n";
            ...
            break;
        case Format::OIL8:
            code << "void " << op_name << "_oil8(const uint8_t* in, float* out, int64_t n) {\n";
            code << indent(1) << "for (int64_t i = 0; i < n; i++) {\n";
...
std::string CodeGenerator::generate_avx2_header() {
    ...
    code << "#define OIL_HAS_AVX2 1\n";
    code << "inline __m256 oil_simd_load(const float* p) { return _mm256_loadu_ps(p); }\n";
    code << "inline void oil_simd_store(float* p, __m256 v) { _mm256_storeu_ps(p, v); }\n";
    code << "inline __m256 oil_simd_add(__m256 a, __m256 b) { return _mm256_add_ps(a, b); }\n";
    code << "inline __m256 oil_simd_mul(__m256 a, __m256 b) { return _mm256_mul_ps(a, b); }\n";
    code << "inline __m256 oil_simd_fma(__m256 a, __m256 b, __m256 c) { return _mm256_fmadd_ps(a, b, c); }\n";
    code << "inline __m256 oil_simd_set1(float v) { return _mm256_set1_ps(v); }\n";
    code << "inline __m256 oil_simd_setzero() { return _mm256_setzero_ps(); }\n";
    code << "inline __m256 oil_simd_max(__m256 a, __m256 b) { return _mm256_max_ps(a, b); }\n";
    code << "inline __m256 oil_simd_exp(__m256 x) {\n";
...
std::string CodeGenerator::generate_avx512_header() {
    ...
    code << "#include <immintrin.h>\n";
    code << "#define OIL_HAS_AVX512 1\n";
    code << "inline __m512 oil_simd512_load(const float* p) { return _mm512_loadu_ps(p); }\n";
    code << "inline void oil_simd512_store(float* p, __m512 v) { _mm512_storeu_ps(p, v); }\n";
    code << "inline __m512 oil_simd512_add(__m512 a, __m512 b) { return _mm512_add_ps(a, b); }\n";
    code << "inline __m512 oil_simd512_mul(__m512 a, __m512 b) { return _mm512_mul_ps(a, b); }\n";
    code << "inline __m512 oil_simd512_fma(__m512 a, __m512 b, __m512 c) { return _mm512_fmadd_ps(a, b, c); }\n";
    code << "inline __m512 oil_simd512_set1(float v) { return _mm512_set1_ps(v); }\n";
    code << "inline __m512 oil_simd512_setzero() { return _mm512_setzero_ps(); }\n";
    code << "inline __m512 oil_simd512_max(__m512 a, __m512 b) { return _mm512_max_ps(a, b); }\n";
    code << "inline __m512 oil_simd512_exp(__m512 x) {\n";
...
std::string CodeGenerator::generate_neon_header() {
    ...
    code << "#define OIL_HAS_NEON 1\n";
    code << "inline float32x4_t oil_neon_load(const float* p) { return vld1q_f32(p); }\n";
    code << "inline void oil_neon_store(float* p, float32x4_t v) { vst1q_f32(p, v); }\n";
    code << "inline float32x4_t oil_neon_add(float32x4_t a, float32x4_t b) { return vaddq_f32(a, b); }\n";
    code << "inline float32x4_t oil_neon_mul(float32x4_t a, float32x4_t b) { return vmulq_f32(a, b); }\n";
    code << "inline float32x4_t oil_neon_fma(float32x4_t a, float32x4_t b, float32x4_t c) { return vmlaq_f32(c, a, b); }\n";
    code << "inline float32x4_t oil_neon_set1(float v) { return vdupq_n_f32(v); }\n";
    code << "inline float32x4_t oil_neon_setzero() { return vdupq_n_f32(0.0f); }\n";
    code << "#else\n";
    code << "#define OIL_HAS_NEON 0\n";
    code << "#endif\n";
    return code.str();
}
...
std::string CodeGenerator::generate_kernel_for_shape(const KernelSpec& spec) {
    ...
    switch (spec.type) {
        ...
        case KernelType::QUANTIZE_DEQUANTIZE:
            code = generate_quantize_dequantize(spec.N, Format::OIL32, spec.format);
            break;
        default:
...
CodeValidationResult CodeGenerator::validate(const std::string& code) {
    ...
    sandbox = fs::path(tmp ? tmp : "C:\\Temp") / "mythos_codegen";
#else
    sandbox = fs::path("/tmp") / "mythos_codegen";
#endif
...
std::string CodeGenerator::generate_full_source(const std::vector<GeneratedKernel>& kernels) {
    std::ostringstream code;
    code << "// Auto-generated OIL kernel collection\n";
    code << "// Generated by MYTHOS CodeGenerator\n";
    code << "#include <cstdint>\n#include <cmath>\n";
    code << "#include <immintrin.h>\n\n";
