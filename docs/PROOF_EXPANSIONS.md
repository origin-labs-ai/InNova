...
### T30.2 Connection to QUANT Weight Partitioning

The QUANT mixed-precision assignment decomposes d weights into three color classes (QUANT8, Ternary, Binary). Define the *sensitivity graph* G_s = (V, E) where:
...
## T31: QUANT KL Divergence — Exact Numerical Constants
...
|--------|-----|----------------|-----------------|---------------|-------------|
| QUANT8   | 256 | 5.5452         | 8.000           | 0.01          | 0.05545     |
...
|----------|---------|------------|-----------|
| √(c_m/2) (QUANT half-CI) | 0.034994 | 0.0312 | 10.8% |
...
For QUANT8 (K=256): σ²_Q_actual = σ²_w · (1 − 1/65536) ≈ σ²_w · 0.99998, while σ²_Q_uniform = Δ²/12 = (6σ_w/256)²/12 = σ²_w · 5.49 × 10⁻⁴.
...
| Theorem 5e (Dead zone) | `test_native_quant.cpp` | Weight indices freeze after 10–20% of training | ✅ Empirically verified |
| Corollary 5e.2 (Stability) | `test_mixed_benchmark.cpp` | ε_QUANT/ε_FP32 ≤ t_avg/T ≈ 0.1–0.2 | ✅ Measured 0.10–0.18 |
...
tests/test_mixed_precision_proof.cpp  (extension)
  ...
  corollary_5a2_dof_bound()
    → Compare n_eff(QUANT) / n_eff(FP32) from PAC-Bayes KL:
      ASSERT ratio < 0.12
