...
### Relevance to QUANT Format

QUANT's format planner currently uses **static** allocation:
- 1% QUANT8, 4% QUANT4, 95% ternary (hardcoded)
...
**QUANT's contribution would be:**
1. First mixed-precision format with ternary + QUANT8 + QUANT4 per-block
...
**This IS genuine research territory** — no paper combines ternary training with QUANT8/QUANT4 VQ in a single format with dynamic routing.
