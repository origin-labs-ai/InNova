...
**For QUANT format specifically:**
- Ternary (3 values) with STE: proven to match FP16 when trained from scratch (BitNet b1.58)
- QUANT8 (256 centroids) with STE: should be EASIER than ternary (more centroids = less quantization error)
- QUANT4 (16 centroids) with STE: moderate difficulty
- Mixed format with STE: unexplored territory — this is where QUANT research is novel

### Practical Implications for QUANT
...
STE is the enabling technology for train-in-format. It's biased but practically effective. For QUANT:
- Ternary blocks: train with STE → proven feasible
- QUANT8 blocks: train with STE → expected feasible (less aggressive than ternary)
- QUANT4 blocks: train with STE → expected feasible
