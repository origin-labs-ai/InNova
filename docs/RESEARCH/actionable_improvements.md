# Actionable Improvements — Extracted from AGI Papers Research

**Task:** DIFFUSION.txt Phase K, Task 136  
**Date:** 2026-07-16  
**Source:** `.research/agi_papers.md`, `.research/quant_idx_research.md`, `.research/distributed_research.md`  
...
**Target file:** `src/inference_opt.cpp`, `include/quant/inference_opt.h`  
...
**Source paper:** Neelakantan et al., "Adding Gradient Noise Improves Learning for Very Deep Networks" (ICLR 2016); grounding for RSI/training stability in `.research/agi_papers.md` §9.  
**Description:** Add a `inject_gradient_noise(eta, gamma)` step in the Trainer, applied **after** gradient clipping and **before** the optimizer step. It adds Gaussian noise `N(0, σ_t²)` with `σ_t = eta / (1 + step)^gamma` to each gradient element. Decaying noise helps escape sharp minima and plateaus during long pretraining and is a cheap, well-validated regularizer. Wire it into `Trainer::fit`/`train_step` behind a `TrainConfig::grad_noise_eta` flag (default 0 = off, so existing tests are unaffected). Uses the existing `quant::RNG` (`include/quant/random.h`) for deterministic, seedable noise.  
**LOC estimate:** ~70 LOC (header config + `trainer.cpp` method + wiring in `fit`/`train_step`)  
**Target file:** `src/trainer.cpp`, `include/quant/trainer.h`  
...
**Source paper:** FlashAttention-3 (Dao/Shah 2024), `.research/agi_papers.md` §1.  
...
**Target file:** `src/inference_opt.cpp`, `include/quant/kv_cache.h`  
...
**Source paper:** Speculative Decoding (Leviathan 2023), `.research/agi_papers.md` §3.  
...
| 5 | Content-Addressed Dedup + Lazy SHA256 | ~200 | `quant_format.cpp/.h` | HIGH | Queued |
