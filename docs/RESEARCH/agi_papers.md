...
| **Application to InNova** | InNova already implements a CPU tiled online-softmax FlashAttention (`src/flash_attention.cpp`, block 64, row_max/row_sum causal). FA-3's contributions map to two engine improvements: (a) **async softmax overlap** — on CPU this translates to prefetching the next K/V tile while reducing the current tile (software pipelining with `_mm_prefetch` / `__builtin_prefetch`), already partially applicable to the existing block loop; (b) **low-precision FP8 accumulation with residual correction** — InNova has `FP8Inference` (`src/inference_opt.cpp:665`) and `KVCache::quantize_fp8_block`; applying FA-3's two-stage FP8 accumulation (keep a running FP32 residual `r` to absorb the FP8 rounding error each block) would make the FP8 attention path numerically safe for causal workloads. The warp-specialization producer/consumer pattern is the blueprint for a future CUDA backend in `include/quant/gpu_compute.h`. |
