...
5. **QUANT8 (256 centroids per block) is inherently lossy** — multiple FP32 values map to same centroid

**For QUANT to achieve "0% loss", it must be train-in-format where the model learns to work within the codebook constraints during training. This is not lossless compression — it's lossless INFERENCE (the model output matches what it would produce given those constraints).**
